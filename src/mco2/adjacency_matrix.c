...
{
    FILE *f;
    String line; // buffer String to hold the ENTIRE LINE

    f = fopen(fileName, "r"); // open the file in read mode
    if (f == NULL)
    {
        printf("Error. File with the name `%s` is not found!", fileName);
    }
    else
    {
